...
import com.gem.hami.util.RandomValidateCode;
...
public class HelpControl {
    ...
    public void selectInfos(String typeId, String curPage, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (typeId1) {
            case 0:
                pageInfo = helpService.findAllHelpsByCreateTime(schoolId, 0, cmap);
                request.setAttribute("helpType", 0);
                break;
            case 1:
                pageInfo = helpService.findBuyInfosByCreateTime(schoolId, cmap);
                request.setAttribute("helpType", 1);
                break;
            case 2:
                pageInfo = helpService.findSendInfosByCreateTime(schoolId, cmap);
                request.setAttribute("helpType", 2);
                break;
            case 3:
                pageInfo = helpService.findFetchInfosByCreateTime(schoolId, cmap);
                request.setAttribute("helpType", 3);
                break;
            case 4:
                pageInfo = helpService.findQueueInfosByCreateTime(schoolId, cmap);
...
//        验证码
        ...
        if(inputCode!=null&&code.equals(inputCode)) {
            ...
            request.getRequestDispatcher("/tian/addHelpBuy/addHelpBuy.jsp").forward(request,response);
            return ;
        }//判定验证码阶段
...
//        System.out.println(name);
    ...
    @RequestMapping(value = "/imageServlet.action")
    public void InvalidateImage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    public void RandomValidateCode(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession  hs = request.getSession();

        String code = (String) hs.getAttribute(RandomValidateCode.RANDOMCODEKEY);
